...
for epoch in range(NUM_EPOCHS):
    for X, y in train_loader:
        ...
        #Update weights
        optimizer.step()
    losses.append(float(loss.data.detach().numpy())) #Detach data from current run and append to list as numpy
...
print(f'Naive classifier accuracy: {np.round(most_common_count/len(y_test)*100)}%')
# %%
